...
function RevealBlock({ session, update }: BlockProps) {
  ...
  return <section className="learning-block reveal-block"><p className="eyebrow">{c.eyebrow}</p><h1>{c.title}</h1><p className="learning-lede">{c.body}</p><div className="lens-system" aria-label={c.expand}>{c.lenses.map((lens, index) => { const active = session.activeLens === index; return <button key={lens.title} className={active ? "active" : ""} aria-expanded={active} onClick={() => update({ activeLens: index })}><span>{String(index + 1).padStart(2, "0")}</span><strong>{lens.title}</strong><em>{lens.question}</em>{active && <span className="lens-detail"><span className="lens-explanation">{lens.explanation}</span><span className="lens-signals"><span><b>{c.buildSignal}</b>{lens.build}</span><span><b>{c.buySignal}</b>{lens.buy}</span></span></span>}</button>; })}</div><button className="primary-action learning-continue" onClick={() => update({}, true)}>{c.cta}<ArrowIcon /></button></section>;
}

function qualitativeLevel(value: number, scale: { low: string; medium: string; high: string }) {
  if (value < 34) return scale.low;
  if (value > 66) return scale.high;
  return scale.medium;
}
...
function VisualBlock({ session, update }: BlockProps) {
  ...
  const tendency = y - x > 22 ? "build" : x - y > 22 ? "buy" : "hybrid";
  ...
  return <section className="learning-block visual-block"><p className="eyebrow">{c.eyebrow}</p><h1>{c.title}</h1><p className="field-intro">{c.body}</p><aside className="field-guide"><strong>{c.howToRead}</strong><p>{c.guide}</p></aside><p className="field-control-hint">{c.controlHint}</p><div className="field-y-heading"><strong>{c.yAxis}</strong><span>{c.yLow} ↑ {c.yHigh}</span></div><div className="decision-field" role="img" aria-label={`${c.tendency}: ${c.tendencies[tendency]}. ${c.interpretation[tendency]}`}><span className="field-label build">{c.build}</span><span className="field-label buy">{c.buy}</span><span className="field-label hybrid">{c.hybrid}</span><i className="field-point" style={{ left: `${x}%`, bottom: `${y}%` }} /></div><div className="field-x-heading"><span>{c.xLow}</span><strong>{c.xAxis}</strong><span>{c.xHigh}</span></div><div className="field-controls"><label><span>{c.xAxis}<output>{qualitativeLevel(x, c.scale)}</output></span><input aria-label={c.xAxis} type="range" min="5" max="95" value={x} onChange={(event) => setX(Number(event.target.value))} /></label><label><span>{c.yAxis}<output>{qualitativeLevel(y, c.scale)}</output></span><input aria-label={c.yAxis} type="range" min="5" max="95" value={y} onChange={(event) => setY(Number(event.target.value))} /></label></div><div className={`field-interpretation ${tendency}`} aria-live="polite"><span>{c.tendency}</span><strong>{c.tendencies[tendency]}</strong><p>{c.interpretation[tendency]}</p><small>{c.caution}</small></div>{!session.fieldPlaced ? <button className="primary-action learning-continue" onClick={savePosition}>{c.firstCta}<ArrowIcon /></button> : <div className="field-reveal"><strong>{c.factsTitle}</strong><ul>{c.facts.map((fact) => <li key={fact}>{fact}</li>)}</ul><p>{c.movePrompt}</p><div className="binary-picker"><button className={moved === true ? "selected" : ""} onClick={() => setMoved(true)}>{c.move}</button><button className={moved === false ? "selected" : ""} onClick={() => setMoved(false)}>{c.keep}</button></div><button className="primary-action learning-continue" disabled={moved === undefined} onClick={() => { update({ fieldX: x, fieldY: y, fieldMoved: moved }, true); }}>{c.cta}<ArrowIcon /></button></div>}</section>;
}
...
function FinalDecisionBlock({ session, update, error, setError }: BlockProps) {
  ...
  const learning = c.learning[session.challenge!.challengeType as keyof typeof c.learning] ?? c.learning.speed;
  ...
  return <section className="learning-block response-block"><p className="eyebrow">{c.eyebrow}</p><h1>{c.title}</h1><p className="learning-lede">{c.body}</p><aside className="challenge-learning"><span>{c.learningTitle}</span><p>{learning}</p></aside><form onSubmit={submit}><h2>{c.prompt}</h2><DecisionPicker value={decision} onChange={setDecision} /><label>{c.rationale}<textarea rows={5} value={rationale} onChange={(event) => setRationale(event.target.value)} placeholder={c.placeholder} /></label>{error && <p className="learning-error">{error}</p>}<ContinueButton disabled={busy}>{c.cta}</ContinueButton></form></section>;
}
