...
export function EconomicsSkillSession({ skillId }: { skillId: string }) {
  ...
  const finish = () => {
    ...
    const applicationPerformance = Math.min(.92, .5 + ((session.applicationResponse?.length ?? 0) / 600) + ((session.challengeResponse?.length ?? 0) / 900));
